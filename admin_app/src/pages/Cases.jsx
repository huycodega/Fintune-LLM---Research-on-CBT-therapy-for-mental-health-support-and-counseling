...
/* Split a reply into its body + the recommended library items so a clinician
   can swap them before approving. The footer is parsed line-by-line and each
   line matched to a library item by EXACT title (kept in footer order), which
   keeps the chips in sync with the user app's identical parser. */
...
function parseFooter(text, library) {
  ...
  for (const line of footer.split("\n")) {
    ...
    if (found && !recs.some((r) => r.kind === found.kind && r.id === found.id)) {
      recs.push(found);
    }
  }
  return { body, recs };
...
function ReviewPanel({ sid, onDone }) {
  ...
  const [adding, setAdding] = useState("");   // selected value in the add picker
      ...
      .then(([L, R]) => {
        setLibrary([
          ...(L.lessons || []).filter((l) => l.status === "published")
            .map((l) => ({ kind: "lesson", id: l.id, title: l.title, duration: l.duration, level: l.level, category: l.category, content: l.content, objectives: l.objectives })),
          ...(R.resources || []).filter((r) => r.status !== "draft")
            .map((r) => ({ kind: "resource", id: r.id, title: r.title, type: r.type, category: r.category, content: r.content, url: r.url })),
        ]);
      });
  }, []);

  // Derived recommendations from the current editable reply.
  ...
  async function act(decision) {
    setBusy(true);
    try {
      // If the clinician changed the selected draft (e.g. added/removed a
      // recommended lesson, or tweaked the text), "Approve" must carry those
      // edits — otherwise the original draft goes out and the changes are lost.
      const draft = (!noDraft && drafts[idx]) ? drafts[idx] : null;
      const changed = draft
        ? (editText.trim() !== (draft.response || "").trim()
           || editTech !== (draft.technique || ""))
        : true;
          ...
          {library.length > 0 && (
            <div className="rec-editor">
              ...
              <div className="rec-chips">
                {recs.map((r) => (
                  <span key={r.kind + r.id} className="rec-chip" title={r.content || ""}>
              ...
              <p className="rec-hint">Edited materials are sent via <b>Send edited reply</b>.</p>
            </div>
          )}
